...
if(!$roomId)
{
    ...
    $ok = true;
    if($stmt->rowCount()==0|| $stmtEmp->rowCount()==0||$stmtAvg->rowCount()==0)
    {
        $ok = false;
...
$title = $state == "ok" ? ("Místnost: ".htmlspecialchars($room->name).""):
         ($state == "bad request" ? htmlspecialchars("Error: Bad Request"):htmlspecialchars("Error: Not Found"));
?>
<!DOCTYPE html>
<html lang="cs">
<head>
...
<body>
    ...
    elseif($state=="bad request")
    {
        ...
        echo die("Error 400: Bad Request");
    } 
    else
    { 
        http_response_code(404);
